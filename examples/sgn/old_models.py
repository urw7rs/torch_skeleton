...
class OldSGN(nn.Module):
    def __init__(self, num_classes, length, num_joints, num_features, bias=True):
        super(SGN, self).__init__()

        self.dim1 = 256
        num_joint = 25

        spa = self.one_hot(1, num_joint, length)
        spa = spa.permute(0, 3, 2, 1)
        self.register_buffer("spa", spa)

        tem = self.one_hot(1, length, num_joint)
        tem = tem.permute(0, 3, 1, 2)
        self.register_buffer("tem", tem)

        self.tem_embed = embed(length, 64 * 4, norm=False, bias=bias)
    ...
    def forward(self, x):
        batch_size, _, num_frames, num_joints, num_features = x.size()
        x = einops.rearrange(x, "n m t v c -> (n m) c v t").contiguous()

        # Dynamic Representation
        dif = x[:, :, :, 1:] - x[:, :, :, 0:-1]
        dif = torch.cat(
            [dif.new(batch_size, dif.size(1), num_joints, 1).zero_(), dif], dim=-1
        )
        pos = self.joint_embed(x)
        tem1 = self.tem_embed(self.tem.repeat(batch_size, 1, 1, 1))
        spa1 = self.spa_embed(self.spa.repeat(batch_size, 1, 1, 1))
